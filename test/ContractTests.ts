...
describe("MutableProxy", function () {
  ...
  it("Should deploy a proxy version 2 with controller1 and call to transparent functions of proxy, and later change implementation", async function () {
    const [wallet] = await ethers.getSigners();
    ...
    const mutableProxyDeployer = await (
      await ethers.getContractFactory("MutableProxyDeployer2")
    ).deploy();
